...
<section class="newsletter-signup {{ $compact ? 'newsletter-compact' : '' }}">
    ...
    <div>
        ...
        <form method="POST" action="{{ route('newsletter.subscribe') }}">
            @csrf
            <label for="newsletter-email">Email address</label><div class="newsletter-email-row"><input id="newsletter-email" type="email" name="email" value="{{ old('email') }}" autocomplete="email" placeholder="you@example.com" required><button class="button" type="submit">Join updates</button></div>
