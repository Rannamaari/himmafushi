...
class DirectoryController extends Controller
{
    public function index(Request $request, string $category)
    {
        ...
        $businesses = Business::public()->where('business_category_id', $categoryModel->id ?? 0)->with('category')->when($search !== '', fn ($query) => $query->where(fn ($nested) => $nested->whereLike('name', "%{$search}%")->orWhereLike('short_description', "%{$search}%")))->orderByDesc('display_priority')->orderByDesc('featured')->orderBy('name')->paginate(12)->withQueryString();
